...
public class ProductSpecification {
    ...
    public static Specification<Product> build(MultiValueMap<String, String> params) {
        ...
        if (params.containsKey("min") && !params.getFirst("min").isBlank()) {
            spec.and(priceGreaterOrEqualsThan(Integer.parseInt(params.getFirst("min"))));
        }

        if (params.containsKey("max") && !params.getFirst("max").isBlank()) {
            spec.and(priceLessOrEqualsThan(Integer.parseInt(params.getFirst("max"))));
        }

        if (params.containsKey("title") && !params.getFirst("title").isBlank()) {
            spec.and(titleLike(params.getFirst("title")));
        }
